...
show_help() {
    ...
    echo " $(basename $0) fix-bug/bug-1 # Create worktree with branch fix-bug/bug-1"
    echo " $(basename $0) --mcp dwh fix-bug/bug-1 # Create worktree and activate dwh MCP template"
...
if [ $? -eq 0 ]; then
    ...
    # Copy credential key files and VS Code tasks if they exist
    echo "Copying credential keys and VS Code settings..."
